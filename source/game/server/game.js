...
export async function getState(db,owner) {
 ...
 return {mode:a.asset==='demo'?'demo':'token',wallet:a.wallet,balance:formatAmount(a.available),rewards:formatAmount(a.rewards),ingots:formatAmount(a.ingots??'0'),locked:formatAmount(a.locked),pool:formatAmount(t.available),burned:formatAmount(t.burned),maxBet:formatAmount(maxBet(BigInt(t.available))),revision:a.revision,rules:policy.rules,benefits:policy.benefits,withdrawalFee:policy.withdrawalFee,referral:await referralStatus(db,a)};
}
export async function play(db,owner,key,amount,{outcomeFactory,minimumInterval=1500,expectedVersion,requireRulesVersion=false}={}) {
 ...
 return operation(db,owner,key,'play',{amount},async(op,now)=>{
  ...
  const policy=await accountPolicy(db,a),rules=policy.rules;
  ...
  if(!outcome||outcomeVersion!==rules.version){outcome=outcomeFactory?outcomeFactory(rules):drawWheel(undefined,rules);outcomeVersion=rules.version;}
  ...
  return {response,statements:[
   ...
   stmt(db,'INSERT INTO rounds(id,owner,operation,bet,gross,fee,net,burn,ingots,score,sequence,rules_version,rules_snapshot,claimed,created_at) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)',roundId,owner,op,String(stake),String(s.gross),String(s.fee),String(s.net),String(s.burn),String(awarded),outcome.score,JSON.stringify(outcome.sequence),rules.version,JSON.stringify({...rules,referral:referral.snapshot}),1,now),
   ...referral.statements,
