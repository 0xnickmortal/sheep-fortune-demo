...
export function sectorText(outcome) {
  ...
  return { label: outcome.multiplierBps / 10000 + '×', shortLabel: String(outcome.multiplierBps / 10000), detail: outcome.id === 'jackpot' ? '万分之一' : outcome.id === 'small-win' ? '小赚' : '返还倍率', kind: outcome.id === 'jackpot' ? 'jackpot' : outcome.multiplierBps >= 20000 ? 'bonus' : outcome.multiplierBps > 10000 ? 'win' : 'payout' };
}
